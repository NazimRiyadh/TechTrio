  ...
  const fetchUsers = async () => {
    try {
      const { data } = await API.get(
        `/api/v1/admin/get-all-users?page=${page}&limit=${limit}`,
      );
      setUsers(data.users || []);
      if (data.pagination) {
    ...
    <div className="admin-users-container">
      ...
      <section className="admin-actions-bar card">
        ...
        <div className="users-stat-quick">
          Total users:{" "}
          <strong className="body-emphasis text-primary">
            {paginationInfo.totalUsers}
          </strong>
        </div>
      </section>
      ...
      <section className="card admin-table-card">
        {loading ? (
          <div
            className="table-loading flex-center"
          ...
          <>
                      ...
                      <tr key={u.id} className={isSelf ? "user-row-self" : ""}>
                        <td>
                          <div className="table-avatar-wrapper">
                            {avatarUrl ? (
                              ...
                              <div
                                className="user-avatar-fallback flex-center"
                                title={u.name}
                              >
                                {getInitials(u.name)}
                              </div>
                        ...
                        <td className="body-emphasis">
                          <div className="user-name-cell">
                            ...
                            {isSelf && (
                              <span className="self-label-pill">You</span>
                            )}
                          </div>
                        </td>
                        ...
                        <td className="text-center">
                          {isSelf ? (
                            <span className="action-note-text caption-sm text-graphite">
                              Protected Account
                            </span>
                          ) : u.role === "admin" ? (
                            <span className="action-note-text caption-sm text-graphite">
            ...
            <div className="admin-users-mobile" style={{ padding: "8px 16px" }}>
              {filteredUsers.map((u) => {
                ...
                return (
                  <div
                    key={u.id}
                    className={`mobile-user-card card ${isSelf ? "user-row-self" : ""}`}
                    style={{
                      padding: 16,
                      marginBottom: 16,
                  ...
                  >
                    ...
                    >
                      <div
                        className="table-avatar-wrapper"
                    ...
                    >
                      <div>
                        ...
                        <span
                          className="caption-md text-graphite"
                          style={{ fontSize: "13px" }}
                        >
                          {formattedDate}
                        </span>
                    ...
                    >
                      {isSelf ? (
                        <span
                          className="action-note-text caption-sm text-graphite"
            ...
            <div className="admin-pagination-bar flex-between">
              <span className="caption-md text-graphite">
                Showing page <strong>{paginationInfo.currentPage}</strong> of{" "}
                <strong>{paginationInfo.totalPages}</strong> (
                {paginationInfo.totalUsers} users total)
              </span>
          ...
          <div className="table-empty-state">
            ...
            <p className="caption-md text-graphite">
              Try adjusting your search filters or page indices.
            </p>
          </div>
        )}
      </section>
